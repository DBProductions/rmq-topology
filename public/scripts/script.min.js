;(function () {
  ...
  const sendExchangeForm = (e) => {
    ...
    if (!error) {
      ...
      document.querySelector('#exchangeTypeSelect').value = 'direct'
      document
        .querySelector('#exchangePanel')
        .classList.remove('panel-wrap-out')
    }
  }
  ...
  const displayBinding = (binding) => {
    ...
    Object.keys(queues).forEach((queue) => {
      if (destinationId === queues[queue].id) {
        selectDestination.options[selectDestination.options.length] =
          new Option(queues[queue].name, queues[queue].id, false, true)
      } else {
        selectDestination.options[selectDestination.options.length] =
          new Option(queues[queue].name, queues[queue].id)
      }
    })
  }
  ...
  document
    .querySelector('#cancelQueueForm')
    .addEventListener('click', hideQueue)
  ...
  const config = {
    description:
      'RMQ Topology helps to simulate and visualize the message flow.',
    producers: [],
    consumers: [],
    exchanges: [],
    queues: [],
  ...
  const findCircle = (val, mx, my) => {
    let found
    if (
      val.constructor.name === 'Exchange' ||
      val.constructor.name === 'Queue'
    ) {
